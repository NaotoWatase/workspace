...
void brown_nkc(){
    ...
    case CHEMICAL:
        turn(180, -60, 0);
        tslp_tsk(200*MSEC);
        straight(28, -80, false, false);
        obj_check(8, RIGHT);
        tslp_tsk(200*MSEC);
        straight(10.5, -80, false, false);
        steering_time(600, -20, 0);
        ...
        if (location[8] != NOTHING) {
            location[8] = CHEMICAL;
            location[9] = NOTHING;
            straight(4.7, 50, false, false);
            chemical_taker(8, LEFT);
            straight(63, 80, true, false);
        }
        else {
            straight(32, -80, false, false);
            //obj_check(9, LEFT);
            location[9] = CHEMICAL;
...
void straight(float cm, float set_power_sign, bool_t savedata, bool_t wall_check) {
    ...
    while (true){
        ...
        if (wall_check == true) {
            ...
            if (wall_length > 11) wall_steer = 4;
            else wall_steer = 0;
        }
        diff = left - right;
        hensa = diff;
        bibun = (hensa - last_hensa) / 0.005;
        steer = diff * p_gein + bibun * d_gein + wall_steer;
...
void water(int n) {
    if (location[n] == FIRE) {
        ...
        if(water_count == 2) {
            ev3_motor_rotate(EV3_PORT_D, 110 , 50, true);
            ev3_motor_rotate(EV3_PORT_D, 20 , 8, true);
            tslp_tsk(100*MSEC);
        ...
        if (obj_distance < 8) {
            switch (obj){
                ...
                case 17:
                    ...
                    if (judgement > 80 || blue > 40 || green > 40 || red > 40 || (h < 200 && h > 100) || (blue - red > 14)) {
                        location[num] = PERSON;
                        if (red - green - blue > 20 || (h > 10 && h < 50 && judgement > 20)) location[num] = FIRE;    
                    }    
                    sensor_check(obj);
                    break; 
                default:
