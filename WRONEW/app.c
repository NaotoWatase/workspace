    ...
    if(sensor == RIGHT){
        ...
        switch (obj){
            ...
            case 0:
                ...
                if (judgement > 14 && judgement < 22) {
                    location[num] = NOTHING;
                    ev3_speaker_play_tone(NOTE_AS5, 200);
                }
                else{
...
void main_task(intptr_t unused){
    ...
    if(chemical != 1)ev3_motor_rotate(EV3_PORT_A, 270, -20, true);
    newsteering(-30, 15);
    p_turn(90, -1, 1);
    newsteering(-30, 10);
    ...
    newsteering(50, 38);
    tank_turn(180, 0, -30);
    steering_time(500, 50, 0);
    if (ev3_motor_get_counts(EV3_PORT_D) > 0) {
        while(true){
            ev3_motor_set_power(EV3_PORT_D, -20);
            if (ev3_motor_get_counts(EV3_PORT_D) < 0)break;
        }
    }
    if (ev3_motor_get_counts(EV3_PORT_D) < 0) {
        while(true){
    ...
    newsteering(-70, 75);
    ev3_motor_rotate(EV3_PORT_D, 90, 40, true);
    p_turn(45, 1, 0);
    p_turn(45, 0, 1);
    ...
    if (map[4] == 1 || map[5] == 1) {
        if (map[4] == 1) {
            ev3_motor_rotate(EV3_PORT_D, 90, 40, true); /*時計回りがプラス*/
            newsteering(50, 5);
