...
/* 音の高さを変える変数(1:通常/2:+1オクターブ/4:+2オクターブ */
...
void main_task(intptr_t exinf) {
    ...
    while (false == isPressed) {
        /* 音をならして、LEDを光らせる */
        /* 音を鳴らす関数の実行には時間がかかる */
        (void)ev3_speaker_play_tone((NOTE_C4 * octaveVal), 500L);
        (void)ev3_led_set_color (LED_RED);

        /* 500ミリ秒待つ */
        tslp_tsk(500*1000);

        (void)ev3_speaker_play_tone((NOTE_D4 * octaveVal), 500L);
        (void)ev3_led_set_color(LED_ORANGE);

        /* 500ミリ秒待つ */
        tslp_tsk(500*1000);

        (void)ev3_speaker_play_tone((NOTE_E4 * octaveVal), 500L);
