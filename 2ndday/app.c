...
void chemical_took(int num, way_t sensor);
...
void steering_wall(int wall_stop, int power, int steering);
...
//　＋以外の角度は存在しない! １８０で真下　0で真上　時計回り（後ろから見て）
...
bool_t sp = false;
bool_t sp_chemical = false;

arm_t arm_type = DOWN;
armsp_t arm_sptype = LEFT_UP;
...
void start_nkc() {
    ...
    /*about_start = ev3_ultrasonic_sensor_get_distance(EV3_PORT_4);
    if (about_start > 20) start = 2;
    else start = 1;*/
    ...
    if(start == 2){
        straight_custom(79, 1, 0, -100);
        turn(90, 80, -80);
    }
    ev3_motor_rotate(EV3_PORT_D, 15, 7, false);
    steering_time(800, -30, 0);
    straight(13.7, 50, false, false);
    ev3_motor_rotate(EV3_PORT_D, 15, -7, false);
    steering_time(800, -30, 0);
    straight(13.7, 50, false, false);
...
void green_nkc() {
    ...
    straight(37.3, 80, true, true);
    obj_check(3, RIGHT);

    steering_time(600, 15, 0);
    if (location[3] == CHEMICAL){
        straight(5.9, -50, false, false);
        turn(90, 50, -50);
...
void brown_nkc(){
    ...
    case CHEMICAL:
        ...
        if (location[8] != NOTHING) {
            ...
            steering_color(COLOR_RED, -20, 0);
            tslp_tsk(400*MSEC);
            straight(8, 48, false, false);
            //obj_check(9, LEFT);
            location[9] = CHEMICAL;
...
void marking_nkc(){
    map_decide();
    //marking
    steering_time(900, 10, 12);
    steering_time(500, 12, 30);
    steering_time(300, 10, -30);
    steering_time(100, 10, 12);
    tslp_tsk(100*MSEC);
    straight(2.5, -25, false, false);
    turn(180, -80, 0);
    tslp_tsk(100*MSEC);
    straight(7, 50, false, false);
    steering_time(800, 23, 0);
    steering_time(200, 15, 0);
    tslp_tsk(300*MSEC);
...
void straight(float cm, float set_power_sign, bool_t savedata, bool_t wall_check) {
    ...
    while (true){
        ...
        if (wall_check == true) {
            ...
            //15 or -10　今までは-10
            if (wall_length > 9.8) wall_steer = -8;
            else wall_steer = 0;
        }
        diff = left - right;
        hensa = diff;
        ...
        if (timing_chemical == 1 && (left / ROBOT1CM > 2.1) && count_chemical == 0) {
            count_chemical = 1;
            arm_up();
        }
...
void water(int n) {
    if (location[n] == FIRE) {
        if(water_count == 1) {
            ev3_motor_rotate(EV3_PORT_D, 67 , 100, true);
            ev3_motor_rotate(EV3_PORT_D, 30 , 15, true);
            tslp_tsk(200*MSEC);
            ev3_motor_rotate(EV3_PORT_D, 97 , -20, false);
        }
        if(water_count == 2) {
            ev3_motor_rotate(EV3_PORT_D, 125 , 100, true);
            ev3_motor_rotate(EV3_PORT_D, 30 , 15, true);
            tslp_tsk(200*MSEC);
            ev3_motor_rotate(EV3_PORT_D, 155 , -20, false);
        }
...
void steering_wall(int wall_stop, int power, int steering){
    ...
    while (wall_stop < wall_length) {
        wall_length = ev3_color_sensor_get_color(EV3_PORT_1);
    }
    (void)ev3_motor_stop(EV3_PORT_B, true);
    (void)ev3_motor_stop(EV3_PORT_C, true);    
}
...
void map_decide(){
    if ((location[8] == NOTHING && location[9] == NOTHING) || location[8] == PERSON || location[9] == PERSON) {
        if (location[8] == PERSON || location[9] == PERSON) person_count = person_count + 1;
        if (person_count <= 1)map[BROWN] = 1;
        marking_count = marking_count + 1;
    }
    if ((location[10] == NOTHING && location[11] == NOTHING) || location[10] == PERSON || location[11] == PERSON) {
        if (location[10] == PERSON || location[11] == PERSON) person_count = person_count + 1;
        if (person_count <= 1){
    ...
    if ((location[5] == NOTHING && location[6] == NOTHING) || location[5] == PERSON || location[6] == PERSON) {
        if (location[5] == PERSON || location[6] == PERSON) person_count = person_count + 1;
        if (person_count <= 1){
    ...
    if ((location[4] == NOTHING && location[7] == NOTHING) || location[4] == PERSON || location[7] == PERSON) {
        if (location[4] == PERSON || location[7] == PERSON) person_count = person_count + 1;
        if (person_count <= 1){
    ...
    if ((location[0] == NOTHING && location[1] == NOTHING) || location[0] == PERSON || location[1] == PERSON) {
        if (location[0] == PERSON || location[1] == PERSON) person_count = person_count + 1;
        if (person_count <= 1){
    ...
    if ((location[2] == NOTHING && location[3] == NOTHING) || location[2] == PERSON || location[3] == PERSON) {
        if (location[2] == PERSON || location[3] == PERSON) person_count = person_count + 1;
        if (person_count <= 1){
...
, 
    ...
    */
    ...
    straight_custom(87, 1, 0, -87);
    steering_time(1000, -20, 0);
    steering_time(200, -10, 0);
    straight(35, 70, false,false);
    turn(90, -80, 80);
    steering_time(800, 15, 0);
    straight(17, -30, false, false);
    ev3_motor_rotate(EV3_PORT_D, 80, 20, true);
    ev3_motor_rotate(EV3_PORT_D, 20, 15, false);
    turn(34, -15, 15);
    straight(14, -80, false, false);
    ev3_motor_rotate(EV3_PORT_D, 100, -25, true);
    straight(14, 80, false, false);
    turn(56, -80, 80);
    straight(20, 80, false, false);
    steering_time(500, 30, 0);
    tslp_tsk(100*MSEC);
    turn(184, 0, -80);
    steering_time(1000, 25, 0);
    steering_time(400, 15, 0);
    ev3_motor_rotate(EV3_PORT_D, 180, 50, true);
    tslp_tsk(100*MSEC);
    straight_custom(85, 1, 0, -100);
    ev3_motor_rotate(EV3_PORT_D, 180, -30, true);
    turn(90, 80, -80);


    steering_time(800, -30, 0);
    straight(13, 50, false, false);
    //waltrace_length(12, 30, 10);
    turn(90, 50, -50);
    steering_color(COLOR_WHITE, 30, 0);
    steering_color(COLOR_BLACK, 24, 0);
    ...
    straight(14, 50, false, false);
    turn(90, -80, 80);
    steering_time(800, -20, 0);
    straight(25, 80, false, false);
    turn(180, 50, -50);
    steering_color(COLOR_WHITE, -15, 0);
    steering_color(COLOR_BLACK, -15, 0);
    ev3_motor_rotate(EV3_PORT_D, 115, 20, true);
    straight(5, -30, false, false);
    turn(90, 50, -50);
    straight(37, -50, false, false);
    turn(90, 0, -50);
    turn(90, 0, 50);
    turn(360, 0, 80);
    ...
    ev3_motor_rotate(EV3_PORT_D, 100, -10, true);
    straight(46, 80, false, false);
    turn(180, 50, -50);
    steering_time(1000, -20, 0);
    straight(12, 50, false, false);
    turn(90, -80, 80);
    steering_time(800, -20, 0);
    ev3_motor_rotate(EV3_PORT_A, 25, 100, true);
    ev3_motor_rotate(EV3_PORT_A, 50, -100, true);
    straight(45, 80, false, false);
    turn(90, 50, -50);
    ...
    straight(30, -80, false, false);
    ev3_motor_rotate(EV3_PORT_D, 90, -30, true);
    straight(80, 80, false, false);
    turn(90, 80, -80);
    straight(40, 80, false, false);
    ev3_motor_rotate(EV3_PORT_D, 300, 30, false);
    steering_time(500, 30, 0);
    straight(10, -50, false, false);
    turn(90, 50, -50);
    straight_custom(105, 1, 0, -100);
    steering_time(800, -30, 0);
    ev3_motor_rotate(EV3_PORT_D, 300, -30, false);
